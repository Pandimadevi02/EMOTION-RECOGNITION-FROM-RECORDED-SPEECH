...
TEST_PRED = np.array([.3, .3, .4, .1, .1, .9, .1])

# page settings
st.set_page_config(page_title="EMOTION RECOGNITION FROM RECORDED SPEECH",
...
def main():
    side_img = Image.open("images/emotion 1.jpg")
    with st.sidebar:
        st.image(side_img, width=300)
        st.title('Emotion Recognition From Recorded Speech')
        st.markdown(
            '<div style="text-align: right;">-by Devi🐰</div>', unsafe_allow_html=True)
    ...
    if website_menu == "Emotion Recognition":
        ...
        model_type = st.sidebar.selectbox(
            "How would you like to predict?", ("mfccs",))
        ...
        txt = """
            This web-application is a part of the final **Data Mining** done by **Harinee** from CSE dept.
        ...
        fig = px.violin(df, y="source", x="emotion4", color="actors",
